...
def create_points(n_domain,n_boundary):
    ...
    Ra_points = np.random.rand(len(y_points))


    return x_points.reshape(-1,1),y_points.reshape(-1,1),Ra_points.reshape(-1,1)
...
def create_wall_points(n_boundary):
    ...
    Ra_points = np.random.rand(len(x_walls_vec))


    return x_walls_vec.reshape(-1,1),z_walls_vec.reshape(-1,1),Ra_points.reshape(-1,1)
...
def plot_loss_history(func,save_path):
    plt.figure(dpi=300)
    loss_plot_start = int(0)
...
def test_compare(pred_model,Ra,Ra_string):
    if Ra == 1e3:
        ...
        psi_mid_point = [0.5,0.5,(np.log10(Ra)-3)/3]
        psi_mid = 1.174
        u_max_z = 0.813
        u_max = 3.649
        u_max_point = [0.5,u_max_z,(np.log10(Ra)-3)/3]
        w_max_x = 0.178
        w_max = 3.697
        w_max_point = [w_max_x,0.5,(np.log10(Ra)-3)/3]
...
def big_plot(xx,yy,outputs,ylen,xlen,Ra_string,save_path,iteration,nepochs):
    ...
    for ii in range(len(plot_list)):
        ...
        plt.title(plot_title[ii],fontsize=30,pad=18)
...
#def BFGS_training(func,parameters,iterations_per_segment):
        ...
        results = tfp.optimizer.bfgs_minimize(value_and_gradients_function=func, initial_position=init_params, max_iterations=iterations_per_segment,parallel_iterations=1)
        func.assign_new_model_parameters(results.position)
def LBFGS_training(func,pred_model,iterations_per_segment):
        init_params = tf.dynamic_stitch(func.idx, pred_model.trainable_variables)
        results = tfp.optimizer.lbfgs_minimize(value_and_gradients_function=func, initial_position=init_params, max_iterations=iterations_per_segment,parallel_iterations=1,num_correction_pairs=100)
        func.assign_new_model_parameters(results.position)