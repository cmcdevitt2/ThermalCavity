...
def function_factory(model,train_x,train_bc,Ra,nepochs,iter_val,history,hist_list):
    ...
    def f(params_1d):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # calculate the loss
            train_x_tensor = tf.transpose(tf.concat([x,z],0))
            ...
            loss_1 = u_vort_dx+w_vort_dz-(vort_xx+vort_zz)*Pr-T_x*Ra*Pr
            loss_2 = 0
            loss_3 = uT_x+wT_z-(T_xx+T_zz)


            """BC Loss Calculations"""
            train_x_bc_tensor = tf.transpose(tf.concat([x_bc,z_bc],0))

            outputs_bc = model(train_x_bc_tensor)
            ...
            # """Output loss"""
            # loss_vec_tensor = tf.transpose(tf.concat((loss_1/Ra,loss_3),axis=0))
            ...
            # """Output loss"""
            # loss_vec_tensor = tf.concat((loss_1/Ra,loss_3),axis=1)
            # #loss_vec_tensor = tf.concat((loss_1,loss_3),axis=1)
            # loss_vec_tensor = tf.reduce_mean(tf.square(loss_vec_tensor))
            # loss_value_tensor = tf.stack((loss_vec_tensor,bc_loss),axis=0)
            # loss_value = tf.reduce_sum(loss_value_tensor)

            # output_loss_vec_tensor = loss_value_tensor

            pde1_loss = tf.reduce_mean(tf.square(loss_1/Ra))
